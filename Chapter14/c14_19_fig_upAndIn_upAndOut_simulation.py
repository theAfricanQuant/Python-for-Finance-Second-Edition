...
for _ in range(0, n_simulation):
    ...
    if outStatus==True:
        ...
        inTotal+=c
        n_in+=1
        S = sp.zeros(n_steps) + barrier
        plt.plot(time_,S,'.-')
        upOutCall=round(outTotal/n_simulation,3)
        upInCall=round(inTotal/n_simulation,3)
        plt.figtext(0.15, 0.8, f'S={s},X={x}')
        plt.figtext(0.15, 0.76, f'T={T},r={r},sigma=={sigma}')
        plt.figtext(0.15, 0.6, f'barrier={barrier}')
        plt.figtext(0.40,0.86, 'call price	='+str(round(c,3)))
        plt.figtext(
            0.40,
            0.83,
            f'up_and_out_call ={str(upOutCall)} (={str(n_out)}/{n_simulation}*{str(round(c, 3))})',
        )
        plt.figtext(
            0.40,
            0.80,
            f'up_and_in_call ={str(upInCall)} (={n_in}/{n_simulation}*{str(round(c, 3))})',
        )
#
plt.title('Up-and-out and up-and-in parity (# of simulations = %d ' % n_simulation +')')
plt.xlabel(f'Total number of steps ={n_steps}')
